...
\title{Um Sistema Baseado em Redes Neurais para Detecção de Intrusão em Tráfego DNS}
\translatedtitle{A Neural Network-Based System for Intrusion Detection in DNS Traffic}

\author[fem]{Giovanna Luisa Hirata dos Anjos}
...
\banca{
  ...
  Prof. Dr. Benjamin Merlin Bumpus -- IME-USP,
  Prof. Dr. Nina S. T. Hirata -- IME-USP,
...
\apoio{Durante o desenvolvimento deste trabalho, o autor recebeu\\
     auxílio financeiro da FAPESP -- processo nº 25/22251-2}

\apoio{During the development if this work, the author received\\
     financial support from FAPESP -- grant \#25/22251-2}
...
\begin{tabular}{rl}
  ...
  ABNT & Associação Brasileira de Normas Técnicas\\
   ...
   EDA & Análise Exploratória de Dados (\emph{Exploratory Data Analysis})\\
   IDS & Sistema de Detecção de Intrusão (\emph{Intrusion Detection System})\\
   IME & Instituto de Matemática, Estatística e Ciência da Computação\\
   KNN & K-vizinhos mais próximos (\emph{K-Nearest Neighbors})\\
   LSTM & Memória de Longo Prazo (\emph{Long Short-Term Memory})\\
   MLP & Perceptron Multicamadas (\emph{Multilayer Perceptron})\\
   SVM & Máquina de Vetores de Suporte (\empty{Support Vector Machine})\\
   TLD & Domínio de nível superior (\emph{Top Level Domain})\\
   TTL & Tempo de Vida (\emph{Time to Live})\\
   UDP & Protocolo de Datagrama do Usuário (\emph{User Datagram Protocol})\\
...
\input{conteudo/00-introducao}
\pagestyle{mainmatter}
\input{conteudo/01-conceitos-basicos}
\input{conteudo/02-revisao-da-literatura}
\input{conteudo/03-investigacao-de-datasets}
\input{conteudo/04-modelo-desenvolvido}
\input{conteudo/05-analise-resultados}
\input{conteudo/06-comparacao-extensao}
\input{conteudo/07-conclusoes}
...
\input{conteudo/apendice-formulario}
\par
